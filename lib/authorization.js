...
const { caught } = require('./exception');
...
const makeTokens = (exports.makeTokens = function(uid, options) {
  ...
  const refreshAssets = encrypt2(
    JSON.stringify({ type: 'refresh_token', uid, expire: refreshExpire })
  );
  return {
    tokenKey: key,
...
const extractCredential = (exports.extractCredential = function(credential, options) {
  if (typeof credential !== 'string') {
    throw new Error(`credential must be string, but got ${typeof credential}.`);
  }
  const opts = Object.assign(
    {
      remix: decrypt,
  ...
  try {
    ...
    if (current - timestamp > opts.expire) {
      throw new Error(
        `credential expired, timeout=${opts.expire}, current=${current},timestamp=${timestamp}.`
      );
    }
    if (origin.token !== token || origin.timestamp.toString() !== timestamp.toString()) {
      throw new Error(`credential not correct, for token or timestamp not match.`);
    }
    return origin;
  } catch (e) {
    throw caught(e, `unexpected error occured when extract credential, details in innerError.`);
  }
});
...
const extractQuery = (exports.extractQuery = function(credential, encryptedQuery, options) {
  ...
  throw new Error('extract credential error occured.');
});
      ...
      .then(function(verifiedUser) {
        ...
        saveTokens(uid, tokens)
          .then(function(result) {
            (result && resolve({ user: verifiedUser, tokens: tokens.tokens })) ||
              reject(new Error('save token error, no result returned.'));
          })
          .catch(caught);
      })
      .catch(caught);
...
exports.handleQuery = function(credential, encryptedQuery, verifyToken, executeQuery, options) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    verifyToken(token)
      .then(function(user) {
        executeQuery({ user, query })
          .then(result => resolve({ user, query, result }))
          .catch(caught);
      })
      .catch(caught);
  });
...
exports.refreshTokens = function(tokens, getUidByAccessToken, verifyAndSaveRefreshToken, options) {
  const { refresh_token, access_token } = tokens;
  return new Promise(function(resolve, reject) {
    const caught = function(err) {
      reject(err);
    };
    getUidByAccessToken(access_token)
      .then(function(uid) {
