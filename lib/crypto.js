...
const defaultEncryptOptions = (exports.defaultEncryptOptions = {
  bit: 6,
...
const encodeByMap = (exports.encodeByMap = function encodeByMap(origin, options) {
  ...
  if (_opts.map.length < Math.pow(2, _opts.bit)) {
    throw new Error(
      `not enough characters in options.map, got ${_opts.map.length}, expected ${Math.pow(
        2,
        _opts.bit
      )}.`
    );
  ...
  if (typeof _keyOrigin === 'string') {
    _keyOrigin = Buffer.from(`##${_last8Length}##${_last2Length}##${_opts.cipher}`).toString(
      'base64'
    );
    key =
      _keyOrigin[_keyOrigin.length - 1] +
      _keyOrigin.substr(1, _keyOrigin.length - 2) +
      _keyOrigin[0];
...
const decodeByMap = (exports.decodeByMap = function(code, options) {
  if (!code) {
    throw caught(
      `param \`code\` must be a string(not empty) or IKeyCodePair { key: string, code: string }, but got [${code}]`
    );
  }
  const _opts = Object.assign(
    {
  ...
  if (typeof _opts.remix !== 'function') {
    throw caught(`options.remix must be a function, but got ${typeof _opts.remix}.`);
  }
  ...
  if (typeof code === 'object' && code.key && code.code) {
    ...
    throw caught(
      'code must be a object with [key] & [code] or a string join key & code with options.join!'
    );
  }
  try {
    _keyOrigin = Buffer.from(
      _keyOrigin[_keyOrigin.length - 1] +
        _keyOrigin.substr(1, _keyOrigin.length - 2) +
        _keyOrigin[0],
      'base64'
    ...
    throw caught(
      e,
      `an error occur when buffer transforming, with _keyOrigin:${_keyOrigin}, _codeOrigin: ${_codeOrigin}.`
    );
  }
  let _str2 = '';
  let _str16 = '';
  ...
  try {
    ...
    throw caught(e, `an error occur when mapping to bits. [_str2: ${_str2}, _int2: ${_int2}]`);
  }
  try {
    // 若有反混淆方法，进行反混淆
    _str2 = remix(_str2);
    ...
    throw caught(
      e,
      `an error occur when remix or grouping codes. [_str2: ${_str2}, _str16: ${_str16}]`
    );
  }
