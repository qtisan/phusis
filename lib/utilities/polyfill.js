...
Exception.prototype.log = function (logger) {
  ...
  logger.error(`[!EXCEPTION] -> [${this.name}(Code:${this.code})] - <line ${this.lineNumber || 0}, col ${this.columnNumber || 0}> - ${this.fileName || 'no file reference'} <----`);
...
Exception.prototype.stacktrace = function (pages, logger) {
  ...
  pages = pages || 1;
  const pageSize = 10;
  let ex = this, max = pageSize * pages, i = 1;
