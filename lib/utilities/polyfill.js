...
function Exception(message, code, error) {
  this.code = code || 0;
  this.innerError = error || this;
  this.message = message || `Here is an unknown error!(${code})`;
}
Exception.prototype = Error.prototype;
Exception.prototype.log = function (logger) {
  logger = logger || __logger__ || console;
  logger.error(`[!EXCEPTION] - [${this.innerError.name}] -> (Code:${this.code}) [line ${this.innerError.lineNumber || 0}, col ${this.innerError.columnNumber || 0}] - ${this.innerError.fileName || 'no file'}<----`);
  this.message && logger.error(`\t [message] - ${this.message}`);
  this.innerError.message && logger.error(`\t [inner] - ${this.innerError.message}`);
  logger.error(`\t ${this.innerError.stack || this.innerError.stack}`);
  logger.error(`\t --------------------------------------------------`);
}
...
function _caught(error, message, code) {
  if (error instanceof Error) {
    return new Exception(
      ...
      typeof message === 'number' ? message : (code || error.number || 0), error
    );
  } else if (typeof error === 'string') {
    return new Exception(
      error, typeof message === 'number' ? message : 0
    );
