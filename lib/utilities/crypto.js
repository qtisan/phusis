...
const decodeByMap = exports.decodeByMap = function (code, options) {
  if (!code) {
    throw caught(`param \`code\` must be a string(not empty) or IKeyCodePair { key: string, code: string }, but got [${code}]`);
  }
  const _opts = Object.assign({
    bit: 5,
    map: 'jtzy_7qfkblovewangxum5dsrh86e3p9',
  ...
  if (_opts.map.length < Math.pow(2, _opts.bit)) {
    throw caught(`not enough characters in options.map, got ${_opts.map.length}, expected ${Math.pow(2, _opts.bit)}.`);
  }
  ...
  if (typeof code === 'object' && code.key && code.code) {
    ...
    throw caught('code must be a object with [key] & [code] or a string join key & code with options.join!');
  }
  try {
  ...
  try {
    ...
    throw caught(e, `an error occur when remix or grouping codes. [_str2: ${_str2}, _str16: ${_str16}]`);
  }
